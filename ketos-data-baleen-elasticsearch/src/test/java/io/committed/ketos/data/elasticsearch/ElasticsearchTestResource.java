...
public class ElasticsearchTestResource {
  ...
  private Client createElasticClient() throws UnknownHostException {
    final Settings settings =
        Settings.builder().put("cluster.name", elasticsearch.getClusterName()).build();

    final InetSocketTransportAddress inetSocketAddress =
        new InetSocketTransportAddress(
            InetAddress.getByName("127.0.0.1"), elasticsearch.getTransportPort());
  ...
  private void loadMappings() {
    final CreateIndexRequestBuilder prepareBuilder =
        client.admin().indices().prepareCreate("testdb");
  ...
  private Map<String, Object> getMapping(String type) {
    ...
    try (InputStream resource =
        getClass().getClassLoader().getResourceAsStream(type + "Mapping.json")) {
      mapping =
          mapper.readValue(resource, new TypeReference<HashMap<String, Map<String, Object>>>() {});
    } catch (final IOException e) {
      fail("Exception when loading test resource:\n" + e.getMessage());
    }
    return mapping;
