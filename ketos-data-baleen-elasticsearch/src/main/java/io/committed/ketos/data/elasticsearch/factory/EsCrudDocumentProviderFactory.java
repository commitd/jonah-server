...
/** A factory for creating ES CrudDocumentProvider. */
@Slf4j
@Service
public class EsCrudDocumentProviderFactory
  ...
  public Mono<CrudDocumentProvider> build(
    ...
    try {
      ...
      final String mentionType =
          (String)
              settings.getOrDefault(
                  "mentionType", BaleenElasticsearchConstants.DEFAULT_MENTION_TYPE);
      final String entityType =
          (String)
              settings.getOrDefault("entityType", BaleenElasticsearchConstants.DEFAULT_ENTITY_TYPE);
      final String relationType =
          (String)
              settings.getOrDefault(
                  "relationType", BaleenElasticsearchConstants.DEFAULT_RELATION_TYPE);
      ...
      return Mono.just(
          new ElasticsearchCrudDocumentProvider(
              dataset, datasource, documents, mentions, entities, relations));
