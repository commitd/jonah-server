...
// return getService().getById(id).map(Converters::toBaleenEntity);
// }
//
// @Override
// public Flux<BaleenEntity> getByDocument(final BaleenDocument document) {
// return getService().getByDocument(document.getId()).map(Converters::toBaleenEntity);
// }
//
// @Override
...
// return getService().countByField(toMentionFilter(filter), path, limit);
// }
//
// @Override
...
//
// @Override
// public EntitySearchResult search(final EntitySearch entitySearch, final int offset, final int
// limit) {
// final Optional<MentionFilter> mentionFilter =
// toMentionFilter(Optional.ofNullable(entitySearch.getEntityFilter()));
// final Flux<OutputEntity> search =
// getService().search(mentionFilter, entitySearch.getMentionFilters(), offset, limit);
