 ...
 *
 * This implementation is based on the ClusteringDocumentList example.
 *
 */
@Service
public class CarrotClusterService {
   ...
   * @return the clusters
   */
  public Mono<Clusters> cluster(final Optional<String> query, final Flux<BaleenDocument> documents) {

    final Map<String, BaleenDocument> idToBaleenDocument = documents.collectMap(BaleenDocument::getId).block();
  ...
  protected Mono<List<Document>> convertToCarrotDocuments(final Collection<BaleenDocument> documents) {
  ...
  protected Clusters convertToClusters(final List<Cluster> clustered,
      final Map<String, BaleenDocument> idToBaleenDocument) {
    final List<Topic> topics = clustered.stream()
