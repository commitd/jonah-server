 ...
 *
 * <p>Authenticated users should be able to add, list, delete their own feedback. Administrative
 * users can list and delete any feedback.
 */
@GraphQLService
public class FeedbackGraphQlService {
  ...
  public FeedbackGraphQlService(
      final DataProviders providers, final List<InvestUiExtension> uiExtensions) {
    this.providers = providers;
    this.uiExtensions = uiExtensions;
  }

  @GraphQLMutation(name = "addFeedback", description = "Save feedback")
  public Mono<Feedback> addFeedback(
    ...
    final Feedback blockLast = getFeedbackProviders().flatMap(d -> d.save(f)).blockLast();
  ...
  public boolean deleteFeedback(
    ...
    if (isAdmin(auth)) {
      getFeedbackProviders().subscribe(d -> d.delete(feedbackId));
    } else {
      getFeedbackProviders().subscribe(d -> d.deleteByUser(feedbackId, auth.getName()));
    }
  ...
  public Flux<Feedback> listFeedback(
      @GraphQLRootContext final InvestRootContext context,
      @GraphQLArgument(name = "offset", description = "Start offset", defaultValue = "0")
          final int offset,
      @GraphQLArgument(name = "size", description = "Maximum values to return", defaultValue = "10")
          final int limit) {
  ...
  private Optional<String> getUsername(final InvestRootContext context) {
    return getUser(context).map(Authentication::getName);
  }

  private Optional<Authentication> getUser(final InvestRootContext context) {
    return context.getAuthentication().filter(Authentication::isAuthenticated).blockOptional();
  }
