...
public class MongoMetadataProvider extends AbstractBaleenMongoDataProvider<OutputDocument>
  ...
  private Optional<Bson> filterByKey(final Optional<String> key) {
    return key.map(
        k -> {
          final String field =
              String.format("%s.%s", BaleenProperties.METADATA, BaleenProperties.METADATA_KEY);
          return Filters.eq(field, key.get());
        });
  }

  protected Flux<TermBin> metadataAggregation(
      final Optional<Bson> filter, final String field, final int size) {
    ...
    aggregation.add(
        Aggregates.project(
            Projections.fields(
                Projections.computed("term", "$_id"), Projections.include("count"))));
