...
public class MongoDocumentProvider extends AbstractBaleenMongoDataProvider<OutputDocument>
  ...
  public DocumentSearchResult search(
      final DocumentSearch documentSearch, final int offset, final int size) {
    if (DocumentFilters.isAggregation(documentSearch)) {
      // IF we have relations / mentions we have to do an aggregation in order to do a join
      // ($lookup).
  ...
  private DocumentSearchResult searchByAggregation(
      final DocumentSearch documentSearch, final int offset, final int size) {

    final List<Bson> aggregation =
        DocumentFilters.createAggregation(
            documentSearch,
            getCollectionName(),
            mentionCollection,
            entityCollection,
    ...
    final Flux<BaleenDocument> results =
        aggregate(aggregation, OutputDocument.class)
            .skip(offset)
            .take(size)
  ...
  private DocumentSearchResult searchByFind(
      final DocumentSearch documentSearch, final int offset, final int size) {
    ...
    final Flux<BaleenDocument> flux =
        toFlux(findPublisher).skip(offset).take(size).map(Converters::toBaleenDocument);
  ...
  public Flux<TimeBin> countByDate(
    ...
    DocumentFilters.createFilter(documentFilter)
        .ifPresent(f -> aggregation.add(Aggregates.match(f)));
    ...
    aggregation.add(
        Aggregates.match(
            Filters.and(
                ...
                Filters.eq(
                    BaleenProperties.PROPERTIES + "." + BaleenProperties.TEMPORAL_PRECISION,
                    BaleenProperties.TEMPORAL_PRECISION_EXACT))));

    // It's hard to pick a sensible data here... this will produce a lot of stuff on the start of
    // month
    // or day as
    // it mentions
    // July 2016 for example. We do have a timestampStart and timestampEnd so we could do something
    // but
  ...
  public Flux<NamedGeoLocation> documentLocations(
    ...
    aggregation.add(
        Aggregates.project(
            Projections.fields(
                Projections.computed("poi", "$" + poiFieldName),
  ...
  private List<Bson> joinCollection(
      final Optional<DocumentFilter> documentFilter, final ItemTypes joinedType) {
    final List<Bson> aggregation = new LinkedList<>();
