...
public class MongoCrudMentionProvider
  ...
  public MongoCrudMentionProvider(
  ...
  public boolean delete(final BaleenMentionReference reference) {
    ...
    delete(
        ...
        Filters.and(
            ...
            Filters.or(
                Filters.eq(
                    BaleenProperties.RELATION_SOURCE + "." + BaleenProperties.EXTERNAL_ID,
                    reference.getMentionId()),
                Filters.eq(
                    BaleenProperties.RELATION_TARGET + "." + BaleenProperties.EXTERNAL_ID,
                    reference.getMentionId()))));

    return delete(
        mentionCollection, filterForMention(reference.getDocumentId(), reference.getMentionId()));
  }
  ...
  public boolean save(final BaleenMention item) {
    ...
    // NOTE We know (that baleen) derives the entity from the mention. But we can't change the
    // entity
    ...
    return replace(
        mentionCollection,
        filterForMention(item.getDocId(), item.getId()),
        outputMention,
        OutputMention.class);
  }
}
