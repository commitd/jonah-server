...
/** Core mention search functionality. */
@GraphQLService
public class MentionSearchService extends AbstractGraphQlService {
  ...
  public Mentions getDocuments(
      @GraphQLContext final MentionSearch search,
      @GraphQLArgument(
            name = "offset",
    ...
    final Flux<MentionSearchResult> results =
        getProviders(optionalCorpus.get(), MentionProvider.class, hints)
    ...
    return Mentions.builder().parent(search).results(flux).total(count).build();
  }
}
