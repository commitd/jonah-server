 ...
 *
 * <p>This is pulled from metadata or generated from the start of the document.
 */
@GraphQLService
public class DocumentSummaryField {
  ...
  @GraphQLQuery(
    name = "summary",
    description =
        "Summary of the content, maybe generated, from metadata or just the start of the text"
  )
  public String title(
    ...
    return summary.length() < size - ELLIPSES.length()
        ? summary
  ...
  private Optional<String> lookInMetadata(final BaleenDocument document) {
    ...
    final Mono<String> keywords =
        document.findAllFromMetadata("keywords").collect(Collectors.joining("; "));
