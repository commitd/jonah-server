...
public class CorpusEntityServiceTest extends AbstractKetosGraphqlTest {
  ...
  @Autowired private CorpusEntityService corpusEntityService;
  ...
  public void testGetById() {
    ...
    postQuery(
            "query($corpus: String!) { corpus(id: $corpus) { entity(id: \"test\") { id } } }",
            defaultVariables())
        .jsonPath("$.data.corpus.entity.id")
        .isEqualTo("test");

    StepVerifier.create(corpusEntityService.getById(getTestCorpus(), "test", null))
        .assertNext(e -> assertNotNull(e.getParent()))
        .verifyComplete();
  }
  ...
  public void testGetEntities() {
    ...
    when(entityProvider.getAll(anyInt(), anyInt()))
        .thenReturn(Flux.fromIterable(Collections.singletonList(entity)));
    postQuery(
            "query($corpus: String!) { corpus(id: $corpus) { entities{ id } } }",
            defaultVariables())
        .jsonPath("$.data.corpus.entities")
        .isArray()
        .jsonPath("$.data.corpus.entities[0].id")
        .isEqualTo("1");
  }
  ...
  public void testGetEntitiesWithProbe() {
    ...
    when(entityProvider.getByExample(any(EntityProbe.class), anyInt(), anyInt()))
        .thenReturn(Flux.fromIterable(Collections.singletonList(entity)));
    postQuery(
            "query($corpus: String!) { corpus(id: $corpus) { entities(probe: {type: \"Person\"}){ id } } }",
            defaultVariables())
        .jsonPath("$.data.corpus.entities")
        .isArray()
        .jsonPath("$.data.corpus.entities[0].id")
  ...
  public void testSearchForEntities() {
    postQuery(
            ...
            defaultVariables())
        .jsonPath("$.data.corpus.searchEntities.entityFilter.type")
        .isEqualTo("Person");
  }
  ...
  public void testCountEntities() {
    ...
    postQuery(
            "query($corpus: String!) { corpus(id: $corpus) { countEntities } }", defaultVariables())
        .jsonPath("$.data.corpus.countEntities")
        .isEqualTo(10);
  }
  ...
  public void testCountByField() {
    ...
    when(entityProvider.countByField(any(), anyList(), anyInt()))
        .thenReturn(Flux.fromIterable(results));
    postQuery(
            "query($corpus: String!) { corpus(id: $corpus) { countByEntityField(field: \"type\"){ bins { term count } } } }",
            defaultVariables())
        .jsonPath("$.data.corpus.countByEntityField.bins")
        .isArray()
        .jsonPath("$.data.corpus.countByEntityField.bins[0].term")
        .isEqualTo("Person")
        .jsonPath("$.data.corpus.countByEntityField.bins[0].count")
        .isEqualTo(1);
  }
  ...
  public void testCountByFieldEmptyField() {
    postQuery(
            ...
            defaultVariables())
        .jsonPath("$.data.corpus.countByEntityField")
        .isEqualTo(null)
